 ...
 * memo：requestBody/requestJson 引用稳定（上层 useMemo）时整块跳过重渲染——
 * 拖面板宽度等高频本地更新不再每帧重建大 Textarea / 递归表格。
 */
const RequestPreview = memo(function RequestPreview({
  requestBody,
  requestJson,
  upstreamEndpoint,
}: {
  ...
  const entries = Object.entries(requestBody).filter(([, v]) => v !== undefined)
