...
**Prompt 节点 @ 引用**：文本框输入 `@` 弹候选菜单（列出与本 Prompt 下游图像/视频节点连线接入的图像/音频/视频资源，含 Prompt 链递归；IME 组词不误触、email 场景有边界判定），
选中插入可读 token `@[显示名]`（素材=文件名 / 生成结果=「label·结果N」，同名消歧）、身份存 `PromptNodeData.mentions`；构建图像/视频请求时替换为 `<<<image_N>>>`/`<<<audio_N>>>`/`<<<video_N>>>` 占位符
（N=资源在实发 image_list/audio_list/video_list 的 1 基序号，列表内容与顺序不因 @ 改变；改名/断线/被 frames 变体裁掉等悬空引用原样保留；Any LLM 请求不替换、token 原文进 prompt）。
...
apps/web/src/
  ...
  lib/requestBody.ts           生成请求体的**单一来源**：buildLlmRequest/buildImageRequest/buildVideoRequest 构造「点击生成时发送的请求 JSON」（收集上游 Prompt/图像/音频/视频 + 按模型整合参数；LLM 请求含 images/audios/videos 多模态输入；图像/视频请求在 prompt 拼接完成后经 applyMentions 把 @[名] 替换成 <<<kind_N>>> 占位符——N 按最终实发列表算，列表本身不动）；节点 handleRun 与 Inspector「请求预览」共用，保证预览=实发不漂移
  ...
  lib/graph.ts                 连线采集：collectUpstreamPrompt（上游 prompt 文本 + 上游 Any LLM 节点输出文本；prompt 有左侧输入→递归并入其上游文本实现 Prompt 链，LLM 结果为终点不回溯，visited 防环；opts.mentionsOut 出参沿途收集各 prompt 节点的 @ 引用映射）/ collectUpstreamImageRefs/AudioRefs/VideoRefs（带来源身份 UpstreamRef{url,nodeId,kind,resultIndex?,label,fileName?} 的采集，供 @ 引用；仅取 URL 的 collectUpstreamImages/Audio/Video 是其 .map(url) 薄封装——图像=上游 image 结果+图像素材、音频=音频素材、视频=video 生成结果+视频素材）/ collectMentionCandidates（@ 菜单候选：从 Prompt 沿下游遍历（prompt 链递归、visited 防环），并入下游图像节点的图像 refs、视频节点的图/音/视三类 refs，按身份键去重 + 显示名消歧）；端点 id 前缀 image-/audio-/video-（imageInputHandleId/audioInputHandleId/videoInputHandleId）
  lib/mentions.ts              @ 引用 token 纯函数：MENTION_TOKEN_RE(@\[名\]) / mentionToken / sanitizeMentionName(剔方括号/控制字符) / uniqueMentionName(同名追加 " (2)") / applyMentions(构建期把可解析 token 替换为 <<<kind_N>>>，N=URL 在实发列表 1 基首现位置；mention/ref/列表任一查不到→原样保留) / pruneMentions(提交文本时清掉 token 已消失的映射，悬空映射保留)
  ...
  components/canvas/
    ...
    nodes/MentionMenu.tsx      @ 引用候选菜单展示组件：自绘绝对定位浮层锚在 Textarea 正下方（**不用 Radix Popover/DropdownMenu——其 pointerdown 会被 React Flow 节点拖拽吞掉**，同预设弹窗的坑）；容器 nodrag/nowheel、菜单项 onMouseDown preventDefault 防失焦；图像项显示缩略图、音/视频显示图标 + 类型徽标
