...
（素材=文件名 / 生成结果=「label·结果N」，同名消歧；**画布上 token 带按资源类型配色的高亮 tag**——Textarea 后方 overlay 层画底色药丸）、身份存 `PromptNodeData.mentions`。
...
apps/web/src/
  ...
  hooks/useMentionMenu.ts      Prompt 节点 @ 菜单逻辑 hook：非组词态输入 @（前一字符非字母数字，防 email 误触）→ 记 anchor + collectMentionCandidates 算候选并打开；继续输入按显示名过滤（跨行/超长自动关）；↑↓/Enter/Tab/Esc（Esc stopPropagation 防 React Flow 取消选中）；select 时同身份复用已有 mention、新身份消歧命名，拼 nextText 后经 onInsert 回调落库
