...
**统一资源端口 + Prompt 节点 @ 引用**：图像/视频生成节点只有一个**统一资源输入端点 `res`**（图/音/视素材与上游生成结果都连这一个口，可连任意多条，无 Add Input 按钮；
...
apps/web/src/
  ...
  store/useFlowStore.ts        Zustand（无 persist）：启动 loadProjects() 拉后端（载入时 normalizeResourceEdges 把旧编号端点资源连线归并到统一端点 res）；增删改调 API；addConnectedNode 从资源源拉线建图像/视频节点时自动连到 res 端点；setProjectPinned(id,pinned) 置顶/取消置顶（照 renameProject 乐观更新 + PUT {pinned}；防抖存盘只 PUT name/nodes/edges 故不会冲掉 pinned）；
