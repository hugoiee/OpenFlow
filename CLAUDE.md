...
**统一资源端口 + Prompt 节点 @ 引用**：图像/视频生成节点只有一个**统一资源输入端点 `res`**（图/音/视素材与上游生成结果都连这一个口，可连任意多条，无 Add Input 按钮；**框选多个资源节点后从其中一个拖线到生成节点可一并连线**——其余选中的合法资源自动连到同一端点，按节点创建序、类型不符的静默跳过；
...
apps/web/src/
  ...
  lib/multiConnect.ts          框选多资源「一并连线」纯函数 collectMultiConnectEdges(nodes,edges,connection)：仅当拖拽起点节点处于选中态才生效（否则是普通单连）；把其余选中的资源节点（sourceKind 为 image/audio/video——即素材 + 图像/视频生成节点）按 nodes 数组序（=创建序，没写 @ 时即实发列表序）连到同一目标端点；经 isValidTypedConnection 过滤不合法的（如音频→只收图像的图像节点）+ 跳过已存在的同源同端点连线；由 useFlowStore.onConnect 调用
  ...
  lib/handleTypes.ts           端点数据类型↔配色 + 连接校验：HANDLE_COLORS（文本粉 / 图像绿 / 音频蓝 / 视频玫红）+ sourceKind/targetAccepts(返回接受类型数组或 null 不限)/edgeColorForSource/isValidTypedConnection（编号端点各接对应类型源、文本端点只接文本源、统一资源端点 res 按节点类型收——图像节点只收图、视频节点收图/音/视，供 FlowCanvas 连线校验与着色）
