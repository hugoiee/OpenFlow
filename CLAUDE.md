...
图像节点的 res 只收图像源、视频节点收图/音/视；旧编号端点 image-N/audio-N/video-N 连线**载入时自动归并**到 res 且保持旧端点序；**例外**：视频「首尾帧」变体保留 First/Last 两个专用图像端点定首/尾帧、Any LLM 节点仍用编号端点 + Add Input）。
...
改名/断线/被筛掉/被裁掉等悬空引用原样保留；Any LLM 请求不替换、token 原文进 prompt）。
...
apps/web/src/
  ...
  lib/requestBody.ts           生成请求体的**单一来源**：buildLlmRequest/buildImageRequest/buildVideoRequest 构造「点击生成时发送的请求 JSON」（收集上游 Prompt/图像/音频/视频 + 按模型整合参数；LLM 请求含 images/audios/videos 多模态输入；图像/视频请求先经 collectMentionedRefs 做 @ 筛选——某类资源被 @ 到则实发列表只含被 @ 的（@ 序）、没 @ 则全发连线资源（连线序），frames 变体 image_list 固定取 First/Last 专用端点前 2 张不受筛选；再经 applyMentions 把 @[名] 替换成 <<<kind_N>>> 占位符——N 按最终实发列表算）；节点 handleRun 与 Inspector「请求预览」共用，保证预览=实发不漂移
  ...
  lib/graph.ts                 连线采集：collectUpstreamPrompt（上游 prompt 文本 + 上游 Any LLM 节点输出文本；prompt 有左侧输入→递归并入其上游文本实现 Prompt 链，LLM 结果为终点不回溯，visited 防环；opts.mentionsOut 出参沿途收集各 prompt 节点的 @ 引用映射）/ collectUpstreamImageRefs/AudioRefs/VideoRefs（带来源身份 UpstreamRef{url,nodeId,kind,resultIndex?,label,fileName?,handle?} 的采集，供 @ 引用与 frames 专用端点区分；仅取 URL 的 collectUpstreamImages/Audio/Video 是其 .map(url) 薄封装——图像=上游 image 结果+图像素材、音频=音频素材、视频=video 生成结果+视频素材）/ collectMentionCandidates（@ 菜单候选：从 Prompt 沿下游遍历（prompt 链递归、visited 防环），并入下游图像节点的图像 refs、视频节点的图/音/视三类 refs，按身份键去重 + 显示名消歧）/ findMentionResource(project,ref)（按 @ 引用身份 nodeId+kind+resultIndex 直查源节点拿资源，供 tag 悬停预览；刻意不复用 collectMentionCandidates 那套 O(E·N) 图遍历——mousemove 路径上太贵，且悬空引用也该能预览到资源本体） / RES_INPUT_HANDLE='res'（图像/视频节点的统一资源输入端点）+ normalizeResourceEdges（载入时把旧编号端点/空 handle 的资源连线归并到 res：按同目标同类型的旧排序键（端点号在前连线序在后）重排子组保证采集次序不变；frames 变体 First/Last 与 LLM 编号端点保留；无需归并原样返回）；端点 id 前缀 image-/audio-/video-（imageInputHandleId/audioInputHandleId/videoInputHandleId，现仅 LLM 节点与视频 frames 变体 First/Last 使用）
