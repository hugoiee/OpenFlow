...
apps/web/src/
  ...
  lib/nodeCatalog.ts           图像/视频预置模型 + 模型→AIGC model_name 映射(IMAGE_API_MODEL/imageApiModel、VIDEO_API_MODEL/videoApiModel) + 各模型可调项选项(图像尺寸/质量/张数、Nano version/宽高比/尺寸、Seedance version/mode/分辨率/时长) + Any LLM 预置模型 LLM_MODELS（现仅作**新建节点默认种子**：Model 选择已改为动态）+ mergeModelOptions(manual, fetched, current)（把手动列表 ∪ 动态获取合并成下拉候选：去空/去重/排序 + 当前已选值置顶；设置面板与节点共用保证一致）+ Temperature 范围 + 配色文案（侧栏与节点共用，含素材节点 ASSET_NODE_META：图像=琥珀 / 音频=天蓝；LLM_NODE_META：紫色）
  ...
  components/inspector/        右侧节点参数面板（Inspector）：节点卡片只显示结果，参数都在此编辑
    ...
    LlmParams.tsx              Any LLM 节点参数：Model **下拉**（候选 = 手动模型列表(设置里维护) ∪ 端点动态获取，经 mergeModelOptions 合并；挂载时自动拉 + 「刷新」按钮重取；已选值置顶保留；并集为空才回退手填输入框）+ Temperature 滑块 + Thinking 开关
apps/desktop/
