...
apps/web/src/
  ...
  store/useFlowStore.ts        Zustand（无 persist）：启动 loadProjects() 拉后端；增删改调 API；setProjectPinned(id,pinned) 置顶/取消置顶（照 renameProject 乐观更新 + PUT {pinned}；防抖存盘只 PUT name/nodes/edges 故不会冲掉 pinned）；
