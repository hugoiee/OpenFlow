...
apps/server/src/
  ...
  provider.ts                  buildImagePayload()/buildVideoPayload()：构造请求体（**独立于发送导出**——失败后要靠同一份请求体去历史按指纹认领，两处各写一遍必然漂移）；runImageGen()/runVideoGen()：POST AIGC 接口生成图像/视频（端点取入参 endpoint，空回退 env AIGC_ENDPOINT）→ GenOutcome{urls,requestId,rawText}；postAigc() 用 res.text() 再 JSON.parse（**不用 res.json().catch(()=>null)**——非 JSON 响应会连原文带状态码一起丢掉，错误退化成字面量 null）；readAigcResult() 按已知契约 `result:{content,status,error{code,message}}` 解析（契约不匹配才回退 extractLegacyUrls 深挖），extractError() **扫到第三层 result.error.message**（本网关错误在那儿，只扫顶层会把「输出音频涉及版权限制」这类真实原因整个吞掉），URL 提取带 exclude 剔除本次请求发出的输入 URL（防回显输入图被当成结果）；EmptyResultError（2xx 但无 URL 且上游没说失败＝可能还在生成，供 task-store 转历史复查）；uploadFiles(form,kind,endpointOverride?)：转发 multipart 到上传接口（kind=image 走图片端点，audio/video 走媒体端点；端点取 override，空回退 UPLOAD_ENDPOINT / UPLOAD_MEDIA_ENDPOINT）；resolveReqFrom()：把全局署名解析成最终 req_from（**为空即抛错、无兜底默认**——req_from 空时不允许发任何上游请求）
...
apps/web/src/
  ...
  lib/nodeCatalog.ts           图像/视频预置模型 + 模型→AIGC model_name 映射(IMAGE_API_MODEL/imageApiModel、VIDEO_API_MODEL/videoApiModel) + 各模型可调项选项(图像尺寸/质量/张数、Nano version/宽高比/尺寸、Seedance version/mode/分辨率/时长)+ mergeModelOptions(manual, fetched, current)（把手动列表 ∪ 动态获取合并成下拉候选：去空/去重/排序 + 当前已选值置顶；供设置面板 Agent 模型名下拉）+ 配色文案（节点与菜单共用，含素材节点 ASSET_NODE_META：图像=琥珀 / 音频=天蓝 / 视频=玫红）
  ...
  lib/nodeMenu.ts              NODE_GROUPS：画布右键菜单点选建节点的分组清单（**建节点入口只有两个**：画布空白右键新建、端点拉线松开在空白处建节点+连线，二者都走这份清单）（按文本/图像/视频三类，含类型/模型/图标/videoVariant）
