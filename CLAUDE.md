...
apps/web/src/
  ...
  lib/mentions.ts              @ 引用 token 纯函数：MENTION_TOKEN_RE(@\[名\]) / mentionToken / sanitizeMentionName(剔方括号/控制字符) / uniqueMentionName(同名追加 " (2)") / collectMentionedRefs(按 @ 出现序收集「被 @ 且仍连线」的资源，身份去重分类型返回——「只发被 @ 的」筛选语义的数据源，悬空不计) / applyMentions(构建期把可解析 token 替换为 <<<kind_N>>>，N=URL 在实发列表 1 基首现位置；mention/ref/列表任一查不到→原样保留) / pruneMentions(提交文本时清掉 token 已消失的映射，悬空映射保留)
